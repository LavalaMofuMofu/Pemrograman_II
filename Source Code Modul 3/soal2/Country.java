...
class Country {
    ...
    public Country(String countryName, String leadershipType, String countryLeader, int independenceDate, int independenceMonth, int year) {
    ...
    public void display(HashMap<Integer, String> month) {
        if (leadershipType.equalsIgnoreCase("monarki")) {
            ...
            String jenis = leadershipType.substring(0, 1).toUpperCase() + leadershipType.substring(1).toLowerCase();
            System.out.println("Negara " + countryName + " mempunyai " + jenis + " bernama " + countryLeader);
            System.out.println("Deklarasi Kemerdekaan pada Tanggal " + independenceDate + " " + month.get(independenceMonth) + " " + year);
        }
        System.out.println();
    }
}